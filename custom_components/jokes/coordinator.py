...
class JokeUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass, config_entry):
        ...
        self.device_basename = config_entry.options.get(
            CONF_DEVICENAME,
            DEFAULT_NAME
        )

        self.device_friendly_name = config_entry.options.get(
            CONF_NAME,
            DEFAULT_NAME
        )

        self.joke_length = config_entry.options.get(
            CONF_JOKE_LENGTH,
            DEFAULT_JOKE_LENGTH
        )
    ...
    async def _async_update_data(self):
        ...
        try:
            device = {
                "id": f"{self.device_basename}_1",
                "uid": f"{self.device_basename}_RJ1",
